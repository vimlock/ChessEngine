...
TEST_CASE("Pawn moves")
{
	...
	SECTION("White double move for on first rank") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A2).count() == 2);

		board.clear();
		board.setSquare(B2, WHITE, PAWN);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(B2).count() == 2);
	}
	...
	SECTION("White double move blocked but single available") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A2).count() == 1);
	}
	...
	SECTION("White blocked") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A2).count() == 0);
	}
	...
	SECTION("Black double move for on first rank") {
		...
		REQUIRE(MoveEval(board, BLACK).getAvailableMoves(A7).count() == 2);

		board.clear();
		board.setSquare(B7, BLACK, PAWN);
		REQUIRE(MoveEval(board, BLACK).getAvailableMoves(B7).count() == 2);
	}
	...
	SECTION("Black double move blocked but single available") {
		...
		REQUIRE(MoveEval(board, BLACK).getAvailableMoves(A7).count() == 1);
	}
	...
	SECTION("Black blocked") {
		...
		REQUIRE(MoveEval(board, BLACK).getAvailableMoves(A7).count() == 0);
	}
	...
	SECTION("White pawn can promote") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A7).count() == 1);
	}
	...
	SECTION("Black pawn can promote and capture") {
		...
		REQUIRE(MoveEval(board, BLACK).getAvailableMoves(A2).count() == 1);
	}
}
...
TEST_CASE("Rook moves")
{
	...
	SECTION("Unblocked moves") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A1).count() == 14);

		board.clear();
		board.setSquare(H1, WHITE, ROOK);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H1).count() == 14);

		board.clear();
		board.setSquare(A8, WHITE, ROOK);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A8).count() == 14);

		board.clear();
		board.setSquare(H8, WHITE, ROOK);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H8).count() == 14);

		board.clear();
		board.setSquare(C2, WHITE, ROOK);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(C2).count() == 14);
	}
	...
	SECTION("blocked moves") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A1).count() == 7);

		board.clear();
		board.setSquare(H1, WHITE, ROOK);
		board.setSquare(G1, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H1).count() == 7);

		board.clear();
		board.setSquare(A8, WHITE, ROOK);
		board.setSquare(A7, WHITE, KNIGHT);
		board.setSquare(B8, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A8).count() == 0);

		board.clear();
		board.setSquare(H8, WHITE, ROOK);
		board.setSquare(H7, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H8).count() == 7);

		board.clear();
		board.setSquare(C2, WHITE, ROOK);
		board.setSquare(C1, WHITE, KNIGHT);
		board.setSquare(C3, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(C2).count() == 7);
	}
}
...
TEST_CASE("Knight moves")
{
	...
	SECTION("On corners") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A1).count() == 2);

		board.clear();
		board.setSquare(H1, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H1).count() == 2);

		board.clear();
		board.setSquare(H8, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H8).count() == 2);

		board.clear();
		board.setSquare(A8, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A8).count() == 2);
	}

	SECTION("One square from corners") {
		board.clear();
		board.setSquare(B2, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(B2).count() == 4);

		board.clear();
		board.setSquare(G2, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(G2).count() == 4);

		board.clear();
		board.setSquare(G7, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(G7).count() == 4);

		board.clear();
		board.setSquare(B7, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(B7).count() == 4);
	}

	SECTION("In middle") {
		board.clear();
		board.setSquare(D4, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(D4).count() == 8);

		board.clear();
		board.setSquare(E5, WHITE, KNIGHT);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(E5).count() == 8);
	}
	...
	SECTION("Blocked by own") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A1).count() == 1);

		board.clear();
		board.setSquare(A1, WHITE, KNIGHT);
		board.setSquare(B3, WHITE, PAWN);
		board.setSquare(C2, WHITE, PAWN);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A1).count() == 0);
	}
...
TEST_CASE("Bishop moves")
{
	...
	SECTION("On corners unblocked") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A8).count() == 7);
	...
	SECTION("One from edge unblocked") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(B2).count() == 9);

		board.clear();
		board.setSquare(G2, WHITE, BISHOP);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(G2).count() == 9);

		board.clear();
		board.setSquare(G7, WHITE, BISHOP);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(G7).count() == 9);

		board.clear();
		board.setSquare(B7, WHITE, BISHOP);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(B7).count() == 9);
	}
	...
	SECTION("In middle unblocked") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(D4).count() == 13);

		board.clear();
		board.setSquare(D5, WHITE, BISHOP);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(D5).count() == 13);
	}
	...
	SECTION("On corners blocked") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H1).count() == 0);
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H8).count() == 0);
	}
	...
	SECTION("On corners blocked but can capture") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H1).count() == 1);
		
		board.clear();
		board.setSquare(A8, WHITE, BISHOP);
		board.setSquare(B7, BLACK, PAWN);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A8).count() == 1);

		board.clear();
		board.setSquare(H8, WHITE, BISHOP);
		board.setSquare(G7, BLACK, PAWN);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H8).count() == 1);
	}
}
...
TEST_CASE("Queen moves")
{
	...
	SECTION("On corners unblocked") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A1).count() == 21);

		board.clear();
		board.setSquare(H1, WHITE, QUEEN);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H1).count() == 21);

		board.clear();
		board.setSquare(H8, WHITE, QUEEN);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H8).count() == 21);
...
TEST_CASE("King moves")
{
	...
	SECTION("On corners unblocked") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(A1).count() == 3);

		board.clear();
		board.setSquare(H1, WHITE, KING);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H1).count() == 3);

		board.clear();
		board.setSquare(H8, WHITE, KING);
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(H8).count() == 3);
	...
	SECTION("King can't move to check") {
		...
		REQUIRE(MoveEval(board, WHITE).getAvailableMoves(D1).count() == 1);
	}
}
...
TEST_CASE("King in check")
{
	...
	SECTION("By pawn") {
		...
		REQUIRE(MoveEval(board, WHITE).isInCheck());
